...
# v29: categorical REMOVED — calibration CSV proved 0/14 wins on 1°C buckets
#      (METAR/station error ±0.5-1°C ≈ bucket width → unsolvable). Only `range`
#      remains, plus a minimum bucket-width filter enforced in edge_calculator.
#      above/below already disabled (not bucket grid).
KINDS_ONLY: List[str] = ["range"]
...
# ── LADDER GRID (головне) ───────────────────────────────────
# v29:PEAK-ONLY AFTER RECALIBRATION — wing YES banned (0/14 wins included wing bets)
SNIPER_GRID_DISTANCE_C: float = 0.5      # PEAK ONLY — was 1.5 (was 4.0 v23-v25 → 0% WR)
WING_BAN: bool = True                     # v29: blocks any BUY_YES where dist > PEAK_DIST_C
...
SNIPER_GRID_MAX_ASK: float = 0.20        # v29: tighter cheap-YES zone (was 0.25)
SNIPER_GRID_NO_MIN_MARKET: float = 0.75  # NO only on extreme misprice (market ≥75¢)
SNIPER_GRID_MAX_PER_CITY_CYCLE: int = 3  # v29: 5→3 (less ladder crawl, tighter fills only)

# v29: Minimum bucket width for `range` markets.
# 1°C categorical-bucket failures proved anything narrower than forecast
# uncertainty is unsolvable with our toolkit. ~3°C = sigma_min at ~12h horizon.
...
# v29: PEAK-ONLY mode — these min-prob gates are enforced only for peak (dist≤0.5°C)
# Wing YES is blocked by WING_BAN in edge_calculator regardless of these floors.
...
# Loaded from data/prob_recalibration.json (generated by calibrate_model.py via
# PAV isotonic regression on trade_log CLOSE records). When ISOTONIC_RECALIBRATE
# is True and the mapping file exists, edge_calculator remaps raw our_prob →
# calibrated prob BEFORE any edge/ratio/Kelly computation.
ISOTONIC_RECALIBRATE: bool = True
ISOTONIC_MAP_FILE: str = "data/prob_recalibration.json"
# Hard floor: any market whose recalibrated our_prob is below this never enters.
# Calibration CSV showed avg_pred 6.6% → actual 2.2%; a 3% floor reflects that
# tail reality and blocks lottery 1¢ buckets that never resolved to WIN.
ISOTONIC_MIN_PROB: float = 0.03
