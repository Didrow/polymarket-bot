"""
main.py — Polymarket Weather Bot v32 (NEAR-RESOLUTION EXECUTION EDGE + LOTTERY EDGE)
...
def main():
    ...
    logger.info("")
    logger.info("=" * 60)
    logger.info("  🌤️  POLYMARKET WEATHER BOT v32  🌤️")
    logger.info("  🎯 NEAR-RESOLUTION EXECUTION EDGE + LOTTERY EDGE (parity layer)")
    logger.info("  📐 Near-res: physical peak-passed buckets, bypass forecast")
    logger.info("  📐 Lottery: cheap peak YES ≤8¢, time-window 00-08 UTC")
    logger.info("=" * 60)
        ...
        if _running:
            ...
            if empty_cycles >= 3:
                if _trader._active_positions:
                    ...
                    now_utc = datetime.now(timezone.utc)
                    # v32: dead-zone tied to OPEN_WINDOW_END_UTC (default 8).
                    ...
                    if win_end <= now_utc.hour or now_utc.hour < 0:
                        ...
                        logger.info(f"🌙 v32 dead-zone ({now_utc.hour:02d}:{now_utc.minute:02d} UTC; window ends {win_end:02d}). "
